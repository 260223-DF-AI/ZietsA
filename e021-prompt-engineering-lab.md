...
5. Document your original prompt, your refined prompt (if needed), and the final output
       ...
       - Original_Prompt: find the top 5 using BigQuery
       - ChatGPT: "Can you clarify what you mean by “top 5 in BigQuery”?"
       - Claude: "What are you into, find the top 5 in BigQuery? Pick three topics to explore." 
       - Gemini: "To find the "top 5" in BigQuery, you generally use the ORDER BY and LIMIT clauses." + gave example of code
       
       - Second_Prompt: find the top 5 deparmtents by average salary including department_name, aberage salary, and employee count
       - ChatGPT: "In Google BigQuery, you can do this with an aggregate query using GROUP BY, then sort and limit to 5." + gace example code
       - Claude: "What are you into, find the top 5 in BigQuery? Pick three topics to explore." 
       - Gemini: "To find the top 5 departments in BigQuery based on their average salary, you need to use the AVG() and COUNT() aggregate functions along with a GROUP BY clause." + gave example of code
...
- Prompt: Explain error - google.api_core.exceptions.BadRequest: 400 Syntax error: 
Expected end of input but got keyword SELECT at [3:1]
- Gemini: "This error occurs because BigQuery encountered a new SELECT statement when it expected the previous one to be finished."
- ChatGPT: "That error from Google BigQuery means the query parser thinks your statement should have already ended—but it unexpectedly encountered another SELECT."
- Claude: "This error occurs because you have a nested/subquery SELECT that BigQuery isn't parsing correctly — usually caused by one of these issues:"
- Rating: 4, all were very helpful and gave a clear explanation and coding examples. There could have been a little more clarity in responses from all of them
...
- Prompt: generate a data dictionary entry for a column 
- ChatGPT: Here’s a clean, reusable data dictionary entry template you can use, followed by a concrete example.
- Gemini: A Data Dictionary is a centralized repository of metadata that describes the meaning, relationships, origin, usage, and format of data.
- Claude: "Also, what is the column name, table, and a brief description of what it stores? (e.g. user_id in orders — unique identifier for the purchasing user)"
- Response Rating: None were very helpful. Would need to include what language I am using, the names of the columns, and what each entry should include. Most responses just wrote a generic data dictionary 
---
